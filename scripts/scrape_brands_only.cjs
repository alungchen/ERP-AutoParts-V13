/**
 * scrape_brands_only.cjs
 * C. 品牌片語表 (Brand_Query2) 完整爬蟲
 */
...
const TARGET_URL  = 'http://cck2.uparts.info/car2009/Brand_Query2/';
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'shorthand_brands.csv');
...
(async () => {
    ...
    try {
        ...
        let fieldMap = { code: 'ccode', fullname: 'cname', extra: 'cnote' };
        if (fields.length >= 2) {
        ...
        const lines = [
            ['代碼', '顯示名', '備註'].join(','),
            ...allData.map(([code, name, note]) => [code, name, note].map(esc).join(','))
        ];
        fs.writeFileSync(OUTPUT_FILE, '\uFEFF' + lines.join('\n'), 'utf8');
        console.log(`📁 已儲存：${OUTPUT_FILE}`);

        console.log('\n📊 前 30 筆預覽：');
        allData.slice(0, 30).forEach(([code, name, note], i) =>
            console.log(`  ${String(i + 1).padStart(3)}. [${(code + '          ').slice(0, 8)}] ${name}  (${note})`)
        );
